...
const RouteAnomalies = () => {
  ...
  return (
    <div className="min-h-screen bg-black p-5 pb-5 lg:p-20">
      <h1 className='font-bold text-3xl text-white'>Route Anomalies</h1>
      <Table className="text-white border-x border-y mt-6">
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-white text-right">Deviation (km)</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {shipments.map((shipment) => (
            <TableRow key={shipment.shipment_id}>
              ...
              <TableCell className="text-right text-white">{shipment.deviation_km}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
